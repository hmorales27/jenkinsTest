...
class Overlord {
    ...
    class NavigationController: UINavigationController {
        ...
        @discardableResult func navigateToViewControllerType(_ type: ViewControllerType, appInfo: CurrentAppInformation) -> Bool {
            var viewControllers: [UIViewController] = []
            var items: [ViewControllerItem] = []
            switch type {
            ...
            case .singleJournal:
                ...
                if publisher.allJournals.count > 1 {
                    let multiJournalVC = MultiJournalViewController(publisher: publisher)
                    items.append(ViewControllerItem(type: .multiJournal, viewController: multiJournalVC))
                    viewControllers.append(multiJournalVC)
                }
                let singleJournalVC = HighlightViewController(journal: journal)
                items.append(ViewControllerItem(type: .singleJournal, viewController: singleJournalVC))
                viewControllers.append(singleJournalVC)
                
            case .topArticles:
                guard let publisher = appInfo.publisher else {
                    return false
                }
                guard let journal = appInfo.journal else {
                    return false
                }
                if publisher.allJournals.count > 1 {
                    let multiJournalVC = MultiJournalViewController(publisher: publisher)
                ...
                let topArticlesVC = TopArticlesMasterVC(journal: journal)
                items.append(ViewControllerItem(type: .topArticles, viewController: topArticlesVC))
                viewControllers.append(topArticlesVC)
            ...
            case .issueTOC:
                ...
                guard let issue = appInfo.issue else {
                    return false
                }
                if publisher.allJournals.count > 1 {
                    viewControllers.append(MultiJournalViewController(publisher: publisher))
                ...
                viewControllers.append(articlesVC)
            ...
            case .issueArticle:
                ...
                guard let article = appInfo.article else {
                    return false
                }
                ...
                let articlePager = ArticlePagerController(article: article, articleArray: articles)
                
                viewControllers.append(articlePager)
            ...
            case .aips:
                ...
                viewControllers.append(AIPVC(journal: journal))
                
            case .aipArticle:
                
                guard let publisher = appInfo.publisher, let journal = appInfo.journal, let article = appInfo.article else {
                    return false
                }
    ...
    internal enum ViewControllerType {
        ...
        static func type(for viewController: UIViewController) -> ViewControllerType {
            ...
            case is MultiJournalViewController:
                return .multiJournal
            case is HighlightViewController:
                return .singleJournal
            default:
                return .unknown
            }
