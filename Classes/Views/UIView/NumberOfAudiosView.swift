...
class NumberOfAudiosView: JBSMView {
    
    let videoImageView = UIImageView(image: UIImage(named: "Audio")!)
    ...
    fileprivate func updatWithCount(_ count: Int) {
        if count == 0 {
            ...
            if count == 1 {
                titleLabel.text = "1 Audio"
                layoutConstraints.width?.constant = width
            } else {
                titleLabel.text = "\(count) Audios"
                layoutConstraints.width?.constant = width
            }
        }
    }
